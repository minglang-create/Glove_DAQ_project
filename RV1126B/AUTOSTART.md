...
**工人**:插好 SD 卡 → 上电 → 程序自动跑起来 → **短按开采** → **长按 2 秒暂停**(段收口 + fsync,
之后可直接断电)→ **短按重新开采**(新开一段)。采集中短按被忽略(防误触);按键永不退出程序;
真要结束 = 长按等落盘后直接断电。日志 `/userdata/glove_daq.log`。
**SD 卡不在或剩余 <2GB = 自检失败**:程序不发 0x5501(STM32 LED 不亮),拒绝开始采集;
插好卡后每秒复检、自动恢复。**拔卡/断电前先长按暂停**(段收口时 fsync;运行中每 5 秒也 fsync,
