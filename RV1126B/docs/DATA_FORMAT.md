...
- 板子自己的 `/userdata/glove_vtime` 记一个 `YYYYMMDDHHMM` 和写它时的 `boot_id`
  (属于**这块采集板**,相当于"本板一共开采过多少次",不跟随 SD 卡;完整重烧固件会清空,之后重新做种并自动避开卡上已有目录);
...
202604131938_seg_001/   本次开机第 1 段
  cam0.h264             左相机 H.264 裸码流(Annex-B, 每帧顺序追加)   ffplay -f h264 cam0.h264
  cam1.h264             右相机 H.264 裸码流    (默认 H.264; 加 -c h265 时为 .h265, 播放用 -f hevc)
...
```
(虚拟时间状态文件在板子上 `/userdata/glove_vtime`,不在卡上;删了会以当前时间重新做种并自动避开卡上已有目录)
...
| `off0` `len0` `off1` `len1` | 该帧在 cam0/cam1.h264 里的**字节偏移与长度** → 可以精确切出"第 N 拍的画面" |
...
pairs.csv 里 cycle = N 的那一行  ←→  glove.csv 里 cycle = N 的那一行
  用 off0/len0 从 cam0.h264 切出画面     用 off 从 glove.bin 读 2690 字节解码
