...
### 1. RJ45 磁座四对镜像反接 → 与百兆设备链不上(千兆直连可用)

> **★2026-09-08 更正★**:同一块板(序列号 2ca7af815e4707b8)与 PC 千兆网卡直连,
> `Link is Up - 1Gbps/Full`,`carrier=1`,可正常配 IP/SSH。说明 1000BASE-T 双方 PHY 能自动
> 纠正四对线序(pair swap),之前"完全不通"的测试对端很可能是百兆设备/交换机(100BASE-TX 只用
> A/B 两对,镜像后落到 D/C 对上,无法协商)。**结论降级**:千兆对端可用,百兆对端不可用;
> V5 仍按下面改法把线序接对,消除这个隐患,但不再是阻塞项(P0 → P1)。
