...
**同步脊柱**：相机晶振是全系统唯一时间基准 → XVS/SYNC 广播给 STM32、5 条 CAN 总线的节点、IMU。
...
- **引脚**（最终以新工程 CubeMX .ioc 为准）：
  ...
  XVS 入=PB10（经 SN74LV1T34 电平转换 1.8→3.3V）；SBU 同步入=PB11；双色 LED=PB1/PB2；
...
- 每节点 = AT32（型号待定，需带 CAN FD）+ MCP2542FD + TMR3111（SPI 读 23bit 角度）+ LDO(5V→3.3V)。
  触觉也是自研 CAN 节点。按手指菊花链，每总线 4~5 节点，**5Mbps 数据段**（8M 已否决），分裂端接。
